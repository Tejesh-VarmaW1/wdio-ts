...
@injectable()
export default class ProductScreen extends Product {
  ...
  async scrollIntoText(searchElement: string): Promise<void> {
    await $(
      `android=new UiScrollable(new UiSelector().scrollable(true)).scrollTextIntoView("${searchElement}")`
    );
  }
  ...
  async back2Products(): Promise<void> {
    await $(
      '//android.view.ViewGroup[@content-desc="test-BACK TO PRODUCTS"]'
    ).click();
  }

  async add2Cart(): Promise<void> {
    await $(
      '//android.view.ViewGroup[@content-desc="test-ADD TO CART"]/android.widget.TextView'
    ).click();
  }
